...
def _process_dataset_background(job_id, dataset_id, steps):
    ...
    try:
        ...
        # Track completion (outside app context since analytics doesn't need DB)
        processing_time = time.time() - processing_jobs[job_id]['start_time']
        # Use a default session ID for background tracking
        session_id = f'background_{job_id}'
        analytics.track_dataset_action(
            session_id=session_id,
            dataset_id=dataset_id,
            action='preprocess',
        ...
        # Handle error - also need app context for error handling
        with app.app_context():
